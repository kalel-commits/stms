...
function setupControlButtons() {
    ...
    document.getElementById('stop-analysis-btn')?.addEventListener('click', stopAnalysis);
...
async function removeVideo(laneId) {
    ...
    // Delete from backend
    try {
        const response = await fetch(`/api/remove-video/${laneId}`, {
            method: 'DELETE'
        });
...
async function startAnalysis() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            addSystemLog('error', err.error || 'Failed to start analysis');
            updateAnalysisStatus('Failed to start analysis');
            analysis_running = false;
        }
    } catch (error) {
        console.error('Error starting analysis:', error);
        analysis_running = false;
        addSystemLog('error', error.message || 'Error starting analysis');
        updateAnalysisStatus('Error starting analysis');
    }
}

async function stopAnalysis() {
    if (!analysis_running) {
        addSystemLog('info', 'Analysis is not running');
        return;
    }
    try {
        const resp = await fetch('/api/stop-analysis', { method: 'POST' });
        if (!resp.ok) {
            const err = await resp.json().catch(() => ({}));
            addSystemLog('error', err.error || 'Failed to stop analysis');
            return;
        }
        analysis_running = false;
        updateAnalysisStatus('Stopped');
        addSystemLog('success', 'Analysis stopped');
    } catch (e) {
        addSystemLog('error', e.message || 'Error stopping analysis');
    }
}

async function resetSystem() {
    // Best-effort stop + backend reset
    try {
        await fetch('/api/reset', { method: 'POST' });
    } catch (_) {
        // ignore network errors; still reset UI
    }
    ...
    if (vehicleChart) {
        ...
        vehicleChart.data.datasets.forEach(d => (d.data = []));
        vehicleChart.update('none');
    }
    if (trafficPieChart) {
        trafficPieChart.data.datasets[0].data = [0, 0, 0, 0];
    ...
    addSystemLog('info', 'System reset');
}
...
function connectWebSocket() {
    ...
    socket.on('disconnect', (reason) => {
        addSystemLog('warning', `Disconnected: ${reason}`);
    });

    socket.on('connect_error', (err) => {
        addSystemLog('warning', `Connection error: ${err?.message || err}`);
    });
        ...
        lanes.forEach(lane => {
            ...
            card.innerHTML = `
                ...
                <div class="video-container" id="video-container-${lane.lane_id}">
                    <video id="video-${lane.lane_id}" src="${lane.video_url}" controls autoplay loop muted playsinline></video>
    ...
    try {
        ...
        if (currentLaneFilter === 'all') {
            ...
            const promises = lanes.map(laneId => 
                fetch(`/api/blockchain/lane/${laneId}`)
                    .then(res => res.json())
            ...
            const response = await fetch(`/api/blockchain/lane/${currentLaneFilter}`);
            const data = await response.json();
